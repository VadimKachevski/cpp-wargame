...
namespace WarGame {
		...
		DemoGame::DemoGame(): board (8, 8) {
			...
			board[{2,0}] = new Paramedic(1);
			board[{1,4}] = new ParamedicCommander(1);
			// board[{1,6}] = new SniperCommander(1);
			assert(board.has_soldiers(1));
		...
		uint DemoGame::play() {
			...
			board.move(1, {2,0}, Board::MoveDIR::Left);      // FootCommander of player 1 moves forward, and all soldiers of player 1 attack.
			if (!board.has_soldiers(2)) return 1;

			// board.move(1,{1,4},Board::MoveDIR::Left);
			// if (!board.has_soldiers(2)) return 1;

			// board.move(1,{1,6},Board::MoveDIR::Left);
			// if (!board.has_soldiers(2)) return 1;
